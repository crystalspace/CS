...
bool
partEdit::Initialize(int argc, const char* const argv[], const char *iConfigName)
{
  ...
  if (kbd == 0)
  {
    csReport (object_reg, CS_REPORTER_SEVERITY_ERROR,
    	"crystalspace.application.partedit2",
  ...
  if (!aws)
  {
    Report(CS_REPORTER_SEVERITY_ERROR, "Could not load the AWS2 plugin!");
    return false;
  }
  ...
  if (nw) nw->SetTitle ("Particle Editor 2.0");
 ...
 if (aws->Load("/partedit2/skin.js")==false)
   Report(CS_REPORTER_SEVERITY_ERROR, "Unable to load the definition file '/partedit2/skin.js'");  
...
partEdit::SetupFrame()
{
  ...
  float speed = (elapsed_time / 1000.0f) * (0.03f * 20);
  ...
  message.Format ("partEdit2(%d)", counter);
...
bool
partEdit::HandleEvent (iEvent &Event)
{
...
void
partEdit::Report (int severity, const char* msg, ...)
{
  va_list arg;
  va_start (arg, msg);
  csRef<iReporter> rep (CS_QUERY_REGISTRY (object_reg, iReporter));
  if (rep)
    rep->ReportV (severity, "crystalspace.application.partedit2", msg, arg);
  else
  {
    csPrintfV (msg, arg);
    csPrintf ("\n");
