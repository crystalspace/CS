	...
	(
	          ...
	          (
	            ...
	            (
	                ...
	                (
	                    ...
	                    format "    <texture name=\"%\">\n" indx[indx.count] to:outFile
	                    renamedfile = indx[indx.count]
	                    renamedfile = substring renamedfile 1 (renamedfile.count-3)+ "mng"
	                    format "      <file>%</file>\n" renamedfile to:outFile
	                    format "    </texture>\n" to:outFile
	      ...
	      (
	        ...
	        (
	            ...
	            (
	                ...
	                -- handles non binary alpha transparent materials
	                if (findString diffuseMapImage "_nba."!=undefined) then
	                (
	                    format "      <shader type=\"depthwrite\">*null</shader>\n" to:outFile
	                )
	
	                -- handles binary alpha transparent materials
	                if (m.mapEnables[7]) then
	                (
	                    format "      <shader type=\"depthwrite\">*null</shader>\n" to:outFile
	                ...
	                (
	                    ...
	                    fogColor = m.selfIllumColor
	                    fogOpacity = m.opacity/100
	                    specularColor = m.specular
	                    specularAlpha = m.specularLevel/100
	                    fogDensity = m.ambient.red/10
	                    perturbX = m.ambient.green/100
	                    perturbY = m.ambient.blue/100
	
	                    format "      <shadervar type=\"vector4\" name=\"water fog color\">%,%,%,%</shadervar>\n" (fogColor.red/255) (fogColor.green/255) (fogColor.blue/255) fogOpacity to:outFile
	                    format "      <shadervar type=\"float\" name=\"water fog density\">%</shadervar>\n" fogDensity to:outFile
	                    format "      <shadervar type=\"vector4\" name=\"specular\">%,%,%,%</shadervar>\n" (specularColor.red/255) (specularColor.green/255) (specularColor.blue/255) specularAlpha to:outFile
	                    format "      <shadervar name=\"water perturb scale\" type=\"vector4\">%,%,0,0</shadervar>\n" perturbX perturbY to:outFile
	                )
	                else
	                (
	        ...
	        if ((classOf m)==Multimaterial) then (
	            ...
	            -- determine if it's a terrain shader material
	            if (findString m.name "_terrainshader_"!=undefined) then (
	                format "    <material name=\"%\">\n" m.name to:outFile
	                format "     <shader type=\"base\">terrain_multisplat</shader>\n" to:outFile
	                format "     <shadervar name=\"specular\" type=\"vector3\">0,0,0</shadervar>\n" to:outFile
	                format "     <shadervar name=\"texture scale\" type=\"vector2\">32,32</shadervar>\n" to:outFile
	                
	                terrain_material = m.name
	
	                currtext = 1
	                for subm in m do (
	                    diffuseMapImage = getMatDiffuseMapFilename subm
	                    -- manage terrain shaders
	                    if (findString subm.name "_shader_" != undefined) then (
	                        if (findString subm.name "_shader_ambient" !=undefined) then (
	                            base_material = diffuseMapImage
	                        )
	                        else if (findString subm.name "_shader_alpha" !=undefined) then (
	                            format "     <shadervar name=\"tex alpha map 1\" type=\"texture\">%</shadervar>\n" diffuseMapImage to:outFile
	                        )
	                        else if (findString subm.name "_shader_terrain" !=undefined) then (
								if (currtext==1) then (
	                                format "     <shadervar name=\"tex diffuse\" type=\"texture\">%</shadervar>\n" diffuseMapImage to:outFile
	                            ) else (
	                                format "     <shadervar name=\"tex diffuse %\" type=\"texture\">%</shadervar>\n" currtext diffuseMapImage to:outFile
	                            )
	                            currtext = currtext + 1
	                        ) else (
	                            message = "ERROR: Found terrain's material "+m.name+" with submat "+subm.name+" defining a shader of unsupported type!\n"
	                            message = message + "Supported types are: _shader_terrain , _shader_alpha"
	                            messageBox message
	                            format "     </material>\n" to:outFile
	                            return 1
	                        )
	                ...
	                -- print basematerial
	                format "    <material name=\"%sshhaaddeerr\">\n" base_material to:outFile
	            ...
	            for subm in m do (
	                ...
	                if (not found) then
	                    ...
	                    if(imagetemp!="materialnotdefined") then
	                    (
	    ...
	    (
	            ...
	            if (type == "particle" and meshfact) then (
	                ...
	                preadvance = getUserProp obj "PREADVANCE"
	                if (preadvance !=undefined) then (
	                    format "  <preadvance>%</preadvance>\n" preadvance to:outFile
	                )
	    ...
	    (
	        ...
	        (
	           ...
	           if (flipModel) then (
	            ...
	            -- check the no hit beam flag
	            hitbeam = getUserProp obj "HITBEAM"
	            if (hitbeam=="no") then (
	              format "      <nohitbeam/>\n" to:outFile
	            )
	            ...
	            if ((classOf obj.mat)==Standardmaterial) then (
	                ...
	                -- check all submaterials
	                  for subm in obj.mat do (                    
	                    if (subm.mapEnables[7]) then (
	                      format "      <trimesh><id>viscull</id></trimesh>\n" to:outFile
	                      format "      <trimesh><default /><id>colldet</id></trimesh>\n" to:outFile
	                      continue;
	    ...
	    (
	        ...
	        if (findString objName "_terr_" !=undefined) then (
	          ...
	          format "   <feeder>crystalspace.mesh.object.terrain2.threadeddatafeeder</feeder>\n" to:outFile
