...
def new_csPolygon3D(*args, **kwargs):
    val = apply(cspacec.new_csPolygon3D,args,kwargs)
    if val: val = iPolygon3DPtr(val)
    return val
...
def new_csView(*args, **kwargs):
    val = apply(cspacec.new_csView,args,kwargs)
    if val: val = iViewPtr(val)
    return val

def new_csTextureHandle(*args, **kwargs):
    val = apply(cspacec.new_csTextureHandle,args,kwargs)
    if val: val = iTextureHandlePtr(val)
    return val
