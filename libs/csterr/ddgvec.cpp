...
ostream& operator << ( ostream&s, ddgVector2 v )
     { return s <<'<'<<v[0]<<','<<v[1]<<'>'; }
ostream& operator << ( ostream&s, const ddgVector2* v )
...
ostream& operator << ( ostream&s, const ddgVector3* v )
...
void ddgVector3::normal( const ddgVector3* v[9])
{
	register int k;
	ddgVector3 n, d[8];
...
ostream& operator << ( ostream&s, const ddgVector4* v )
...
ddgVector4* ddgVector4::multiply( const ddgMatrix4 *m1, const ddgVector4 *v1 )
{
  for (int i=0; i<4; i++) {
    v[i] = 0;
    for (int j=0; j <4; j++) v[i] += m1->v[i*4+j] * v1->v[j];
    }
  return this;
}
#endif
