...
csCBuffer::csCBuffer (int sx, int ex, int n_lines)
{
  ...
  int i;
  for (i = 0; i < num_lines; i++) lines[i].SetParent (this);
...
{
  int y;
  for (y = 0; y < num_lines; y++) lines[y].Initialize (startx, endx);
...
bool csCBuffer::TestPolygon (csVector2 *verts, int num_verts)
{
  ...
  for (;;)
  {
    ...
    do
    {
      ...
      {
        scanL1 = scanL2;
        if (--scanL2 < 0) scanL2 = num_verts - 1;
...
{
  return lines[QRound (point.y)].TestSpan (
      QRound (point.x),
      QRound (point.x));
}
