...
{
  int i;
  for (i = 0; i < 8; i++) delete children[i];
  delete minibsp;
...
{
  delete[] minibsp_verts;
  if (minibsp) minibsp_verts = minibsp->GetVertices (minibsp_numverts);
...
csOctree::csOctree (
  csThing *thing,
  const csVector3 &imin_bbox,
  const csVector3 &imax_bbox,
  int ibsp_num,
  int imode) :
    csPolygonTree(thing)
{
...
void csOctree::ChooseBestCenter (
  csOctreeNode *node,
  csPolygonInt **polygons,
  int num)
{
  ...
  int num)
{
  ...
  int i;
  for (i = 0; i < num; i++) node->unsplit_polygons.AddPolygon (polygons[i]);
  ...
  void *culldata)
{
  ...
  if (node->GetMiniBsp ())
    return node->GetMiniBsp ()->Back2Front (
        pos,
        func,
        data,
        cullfunc,
  ...
  void *culldata)
{
  ...
  if (node->GetMiniBsp ())
    return node->GetMiniBsp ()->Front2Back (
        pos,
        func,
        data,
        cullfunc,
...
void csOctree::Statistics ()
{
  ...
  int tot_pvs_vis_nodes = 0, min_pvs_vis_nodes = 1000000000,
    max_pvs_vis_nodes = 0;
  int tot_pvs_vis_poly = 0, min_pvs_vis_poly = 1000000000,
    max_pvs_vis_poly = 0;
  best_pvs_node = NULL;
  Statistics (
    (csOctreeNode *)root,
  ...
  e->Report (
      "  oct_nodes=%d max_oct_depth=%d num_bsp_trees=%d",
      num_oct_nodes,
      max_oct_depth,
      num_bsp_trees);
  e->Report (
      "  bsp nodes: tot=%d avg=%d min=%d max=%d",
      tot_bsp_nodes,
      avg_bsp_nodes,
      min_bsp_nodes,
      max_bsp_nodes);
  e->Report (
      "  bsp leaves: tot=%d avg=%d min=%d max=%d",
      tot_bsp_leaves,
      avg_bsp_leaves,
      min_bsp_leaves,
      max_bsp_leaves);
  e->Report (
      "  bsp max depth: tot=%d avg=%d min=%d max=%d",
      tot_max_depth,
      avg_max_depth,
      min_max_depth,
      max_max_depth);
  e->Report (
      "  bsp tot poly: tot=%d avg=%d min=%d max=%d",
      tot_tot_poly,
      avg_tot_poly,
      min_tot_poly,
      max_tot_poly);
  ...
  int avg_pvs_vis_nodes = num_pvs_leaves ? tot_pvs_vis_nodes /
    num_pvs_leaves : 0;
  int avg_pvs_vis_poly = num_pvs_leaves ? tot_pvs_vis_poly /
    num_pvs_leaves : 0;
  int tot_polygons = ((csOctreeNode *)root)->CountPolygons ();
  e->Report (
      "  pvs vis nodes: tot=%d avg=%d min=%d max=%d",
      tot_pvs_vis_nodes,
      avg_pvs_vis_nodes,
      min_pvs_vis_nodes,
      max_pvs_vis_nodes);
  e->Report (
      "  pvs vis poly: avg=%d%% min=%d%% max=%d%%",
  ...
  if (best_pvs_node)
  {
    ...
    e->Report ("  best pvs node at %f,%f,%f", center.x, center.y, center.z);
  }
}
...
void csOctree::Statistics (
  ...
  int *max_tot_poly,
  int *num_pvs_leaves,
  int *tot_pvs_vis_nodes,
  int *min_pvs_vis_nodes,
  int *max_pvs_vis_nodes,
  int *tot_pvs_vis_poly,
  int *min_pvs_vis_poly,
  int *max_pvs_vis_poly)
{
  if (!node) return ;
  depth++;
  ...
  if (node->IsLeaf ())
  {
    ...
    if (num_vis_nodes > *max_pvs_vis_nodes)
      *max_pvs_vis_nodes = num_vis_nodes;
    if (num_vis_nodes < *min_pvs_vis_nodes)
      *min_pvs_vis_nodes = num_vis_nodes;
  ...
  if (node->GetMiniBsp ())
  {
    ...
    node->GetMiniBsp ()->Statistics (
        &bsp_num_nodes,
        &bsp_num_leaves,
        &bsp_max_depth,
        &bsp_tot_polygons,
        &bsp_max_poly_in_node,
        &bsp_min_poly_in_node);
