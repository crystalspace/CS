...
ctGravityF::ctGravityF( ctReferenceFrame &rf, real pg, ctVector3 pd ) : ctForce( rf )
{
	magnitude = pg, direction = pd; 
}
...
ctVector3 a2;
	...
	if( body_vector.get_size() == 2 && attachment_point_vector.get_size() == 2 ){
		...
		if( b1 && b2 && orig_a1 && orig_a2 ){
			...
			if( r1 && r2 ){
				...
				if( &pe == b1 ){ 
					...
					Debug::log( CT_DEBUG_LEVEL, "ctSpringF: body applied not part of coupling\n" );
