...
struct iGraphics3D : public virtual iBase
{
  ...
  */
   ...
   */
  virtual void OQInitQueries (unsigned int* queries, int num_queries) const = 0;

  virtual void OQDelQueries (unsigned int* queries, int num_queries) const = 0;

  /**
   * Returns whether an occlusion query has finished.
   */
  virtual bool OQueryFinished (unsigned int occlusion_query) const = 0;

  /**
   * Check via occlusion query whether a mesh is visible.
   */
  virtual bool OQIsVisible (unsigned int occlusion_query, unsigned int sampleLimit) const = 0;

  virtual void OQBeginQuery (unsigned int occlusion_query) const = 0;
  virtual void OQEndQuery () const = 0;
