...
public:
	...
	/// Track memory usage.
	static void memory( const char * file, unsigned int line, unsigned long size, unsigned long count, const char *type);
#ifdef _DEBUG
...
#else
	static bool reportMemory( ddgStr *filename = NULL, char *s = NULL ) { return false; }
#endif
	/// Report the total recorded memory in used.
#ifdef _DEBUG
	static unsigned long reportTotalInUse(void);
#else
	static unsigned long reportTotalInUse(void) { return 0; }
#endif
	/// Global defines
