...
public:
	...
	/// Construct with initial values from another vector.
	ddgVector3( ddgVector3* u )                { v[0] = u->v[0]; v[1] = u->v[1]; v[2] = u->v[2]; }
	/// Assign to another vector.
	...
	///
	void set( ddgVector3 *u )                { v[0] = u->v[0]; v[1] = u->v[1]; v[2] = u->v[2]; }
    ///
    void get(float *a, float *b, float *c) { *a = v[0]; *b = v[1]; *c = v[2]; }
	///
	void add( ddgVector3 *u )                { v[0] += u->v[0]; v[1] += u->v[1]; v[2] += u->v[2]; }
	/// Add two vectors
	inline friend ddgVector3 operator+( const ddgVector3& u, const ddgVector3& w) 
		{ return ddgVector3(u[0]+w[0],u[1]+w[1],u[2]+w[2]); }
//	void add( float* u )                 { v[0] += u[0]; v[1] += u[1]; v[2] += u[2]; }
	///
	void subtract( ddgVector3 *u )           { v[0] -= u->v[0]; v[1] -= u->v[1]; v[2] -= u->v[2]; }
	/// Subtract two vectors
	inline friend ddgVector3 operator-( const ddgVector3& u, const ddgVector3& w) 
		{ return ddgVector3(u[0]-w[0],u[1]-w[1],u[2]-w[2]); }
	///
//	void subtract( float* u )            { v[0] -= u[0]; v[1] -= u[1]; v[2] -= u[2]; }
	///
	...
	inline friend ddgVector3 operator*( const ddgVector3& u, const ddgVector3& w) 
		{ return ddgVector3(u[0]*w[0],u[1]*w[1],u[2]*w[2]); }
	///
//	void multiply( float* u )            { v[0] *= u[0]; v[1] *= u[1]; v[2] *= u[2]; }
	/// Multiply his this and a vector.
	void multiply( ddgVector3 *u )            { v[0] *= u->v[0]; v[1] *= u->v[1]; v[2] *= u->v[2]; }
	///
	void divide( float s )               { float s2=1.0f/s; multiply(s2); }
	///
//	void divide( float* u )              { v[0] /= u[0]; v[1] /= u[1]; v[2] /= u[2]; }
	///
	void divide( ddgVector3* u )              { v[0] /= u->v[0]; v[1] /= u->v[1]; v[2] /= u->v[2]; }
	/// Divide two vectors
	inline friend ddgVector3 operator/( const ddgVector3& u, const ddgVector3& w) 
		{ return ddgVector3(u[0]/w[0],u[1]/w[1],u[2]/w[2]); }
	///
	...
	void normalize( void )               { divide(size()); }
	/// Returns the angle between 2 normalized vectors.
	float angle( ddgVector3 *u )            { return ddgAngle::radtodef(acosf(dot(u))); }
    ...
    void vectortoangle( void )
    {
		...
		{
			yaw = (float)(int) 90+ddgAngle::radtodef(atan2f(v[2], v[0]));
			yaw = ddgAngle::mod(yaw);

			pitch = (float)(int) ddgAngle::radtodef(atan2f(v[1], sqrtf(v[0]*v[0] + v[2]*v[2])));
	...
	void cross( ddgVector3 *u, ddgVector3 *w )
    {
	...
	void normal( ddgVector3 *p0, ddgVector3 *p1, ddgVector3 *p2 )
    {
		ddgVector3 u(p1), v(p2);
		u.subtract(p0);
		v.subtract(p1);
		cross(u,v);
    }
	 ...
	 * Renderman spec.
	 */
	void refract( ddgVector3 *n, float eta )
    {
		float IdotN = dot(n);
...
public:
	...
	/// Test for equivalence.
	bool operator==(ddgVector4 u) { return v[0] == u[0] && v[1] == u[1] && v[2] == u[2] && v[3] == u[3]; }
	/// Test for inequivalence.
	bool operator!=(ddgVector4 u) { return v[0] != u[0] || v[1] != u[1] || v[2] != u[2] || v[3] != u[3]; }
	///
	ddgVector4(void )                                { }
	...
	///
	ddgVector4( ddgVector4* u )                         { v[0] = u->v[0]; v[1] = u->v[1]; v[2] = u->v[2]; v[3] = u->v[3];}
	///
	ddgVector4( ddgVector3* u )                         { v[0] = u->v[0]; v[1] = u->v[1]; v[2] = u->v[2]; v[3] = 0;}
	///
	...
	///
	void set( ddgVector3 *u )                        { v[0] = u->v[0]; v[1] = u->v[1]; v[2] = u->v[2]; v[3] = 0;}
	///
	void set( ddgVector4 *u )                        { v[0] = u->v[0]; v[1] = u->v[1]; v[2] = u->v[2]; v[3] = u->v[3];}
    ///
    void get(float *a, float *b, float *c, float *d) { *a = v[0]; *b = v[1]; *c = v[2]; *d = v[3]; }
	///
	void add( ddgVector4 *u )                         { v[0] += u->v[0]; v[1] += u->v[1]; v[2] += u->v[2]; v[3] += u->v[3]; }
	/// Add two vectors
	inline friend ddgVector4 operator+( const ddgVector4& u, const ddgVector4& w) 
		{ return ddgVector4(u[0]+w[0],u[1]+w[1],u[2]+w[2],u[3]+w[3]); }
	///
//	void add( float* u )                          { v[0] += u[0]; v[1] += u[1]; v[2] += u[2]; v[3] += u[3]; }
	///
	void subtract( ddgVector4 *u )                    { v[0] -= u->v[0]; v[1] -= u->v[1]; v[2] -= u->v[2]; v[3] -= u->v[3]; }
	/// Subtract two vectors
	inline friend ddgVector4 operator-( const ddgVector4& u, const ddgVector4& w) 
		{ return ddgVector4(u[0]-w[0],u[1]-w[1],u[2]-w[2],u[3]-w[3]); }
	///
//	void subtract( float* u )                     { v[0] -= u[0]; v[1] -= u[1]; v[2] -= u[2]; v[3] -= u[3]; }
	///
	void multiply( float s )                      { v[0] *= s; v[1] *= s; v[2] *= s; v[3] *= s; }
	///
//	void multiply( float* u )                     { v[0] *= u[0]; v[1] *= u[1]; v[2] *= u[2]; v[3] *= u[3]; }
	///
	...
	///
//	void divide( float* u )                       { v[0] /= u[0]; v[1] /= u[1]; v[2] /= u[2]; v[3] /= u[3]; }
	///
	void divide( ddgVector4* u )                     { v[0] /= u->v[0]; v[1] /= u->v[1]; v[2] /= u->v[2]; v[3] /= u->v[3];}
	/// Divide two vectors
	inline friend ddgVector4 operator/( const ddgVector4& u, const ddgVector4& w) 
		{ return ddgVector4(u[0]/w[0],u[1]/w[1],u[2]/w[2],u[3]/w[3]); }
	///
