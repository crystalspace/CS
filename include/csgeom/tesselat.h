...
/**
 *
 * The method behind the tesselator is a very simple structure of a
 * point interpolation algorithm and two tables containing the edge
 * results of the cube filter and a triangle interpretation table to
 * complete the mapping. Upon entry ( after initializing values ),
 * the algorithm first evaluates each corner of the grid cell to test
 * if it is greater or less than zero. For each corner that is less
 * than zero, an index flag is set on the edge index  (cubeindex).
 * It is probably not without coincidence that the number of corner
 * flags in a cube neatly fits into a single byte. 
 *
 * The edge table represents a vertex outcome based on the the corner
 * map it was supplied. It is a twelve bit mapping respresenting each
 * edge of the cube. The order is specifc, and these are the edge 
 * vertices. (this table is based on the one earlier on)
 *
 *<pre>
 *  bit 0: p[0] - p[1]
 *  bit 1: p[1] - p[2]
 ...
 *</pre>
 *
 * For each state where one vertex is greater than zero and its nieghbour is
 * is less than zero, a corresopnding flag is set by the edge table. After
 * all edges that incur an intersection are set, the next step is interpolating
 * the intersection vertices. The interpolation algorithm uses the values
 * of each corner (GridCell.val[]) to produce a linear estimate of where the
 * intersection with the edge occurs. This is not 100% accurate, but does
 * provide a nice approximation.
 * 
 * The triangle table is a scripted set of triangle maps based on
 * the cube index. It contains 256 mappings with up to five
 * triangles. The sixteenth element of every mapping is -1, to
 * terminate the mapping. In many cases, the map table is filled with
 * terminations after two or three triangles. Now although there are only
 * twelve resultant vertices from the interpolation unit, in some cases,
 * a given edge vertex may be used up to four times in a given mapping,
 * resulting in a maximum of fifteen vertices in the output.
 *
 */
