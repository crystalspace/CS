...
/// A csWeakRefHash<> that maintains a reverse hash for indexing keys by values.
template <class T, class K = unsigned int>
class csWeakRefHashReversible : public csWeakRefHash<T, K>
{
  csHash<K, csWeakRef<T>> reverse;
public:
  /**
   ...
   */
  csWeakRefHashReversible (int size = 23, int grow_rate = 5, int max_size = 20000) :
    csWeakRefHash<T, K> (size, grow_rate, max_size), 
  ...
  /**
    ...
    */
  void Put (const K& key, const csWeakRef<T> &value)
  {
    csWeakRefHash<T, K>::Put (key, value);
  ...
  /**
    ...
    * @remarks 
    */
  void PutUnique (const K& key, const csWeakRef<T> &value)
  {
    csWeakRefHash<T, K>::PutUnique (key, value);
   ...
   */
  const K* GetKeyPointer (const csWeakRef<T>& key) const
  {
   ...
   * none.
   */
  const K& GetKey (const csWeakRef<T>& key, const K& fallback) const
  {
