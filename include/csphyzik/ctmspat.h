...
public:
  ...
  void add3( ctSpatialMatrix6 &pmdest, const ctSpatialMatrix6 &pm1, const ctSpatialMatrix6 &pm2 ){
    for( int idx = 0; idx < 6; idx++ )
      for( int idy = 0; idy < 6; idy++ )
  ...
  void subtract3( ctSpatialMatrix6 &pmdest, const ctSpatialMatrix6 &pm1, const ctSpatialMatrix6 &pm2 ){
    for( int idx = 0; idx < 6; idx++ )
      for( int idy = 0; idy < 6; idy++ )
        pmdest.rows[idx][idy] = pm1.rows[idx][idy] - pm2.rows[idx][idy];
  }
...
inline ctSpatialMatrix6 ctSpatialVector6 ::operator*( const ctVectorTranspose6 &pv )
{ 
ctSpatialMatrix6 Mret;
  for( int idr = 0; idr < 6; idr++ ){
    for( int idc = 0; idc < 6; idc++ ){
