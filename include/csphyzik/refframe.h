...
//		return T_world.This2Other( v );
	...
	const ctMatrix3 &get_R(){ return fTg; }
	const ctMatrix3 &get_this_to_parent(){ return fTg; }
	void set_R( const ctMatrix3 &M ){ fTg = M;  gTf = fTg.get_transpose(); }
	void set_this_to_parent( const ctMatrix3 &M ){ fTg = M; gTf = fTg.get_transpose(); }

	// get/set tranform matrix ( transfrom from parent to child frame )
	const ctMatrix3 &get_T(){ return gTf; }
	const ctMatrix3 &get_parent_to_this(){ return gTf; }
	void set_T( const ctMatrix3 &M ){ gTf = M; fTg = gTf.get_transpose(); }
	void set_parent_to_this( const ctMatrix3 &M ){ gTf = M; fTg = gTf.get_transpose(); }
...
protected:
	...
	ctMatrix3 fTg;	// transfrom from this frame to parent frame
	ctVector3 offset;  // offset from parent frame to this one in parent's frame
