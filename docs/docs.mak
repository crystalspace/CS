...
.PHONY: devapi pubapi htmldoc dvidoc psdoc pdfdoc infodoc repairdoc cleandoc
.PHONY: do-devapi do-pubapi devapi-postprocess pubapi-postprocess
...
# Rule to post-process HTML files generated by Doxygen.  Remove hidden,
# embedded date (which would otherwise foul up automated conversion and CVS
# update).  Also rename from .html to .htm.

%.htm: %.html
	@$(SED) \
	-e 's:Generated by \(..*\) on \(..*\) -->:Generated by \1 -->:g' \
	-e 's:\.html:\.htm:g' \
	-e 's:doxygen/index\.htm:doxygen/index\.html:g' < $< > $@

# Rules to generate developer API documentation from all header files.
devapi-postprocess: $(API.DEV.HTM.FILES)
	@$(RM) $(API.DEV.HTML.FILES)

do-devapi: $(OUT.DOC.API.DEV)
	$(DOXYGEN) $(DOXYGEN_DEVAPI)

devapi: $(OUT.DOC.API.DEV).CLEAN do-devapi
	@echo "Postprocessing Doxygen output."
	@$(MAKE) --no-print-directory -f mk/cs.mak devapi-postprocess

# Rules to generate public API documentation from public header files.
pubapi-postprocess: $(API.PUB.HTM.FILES)
	@$(RM) $(API.PUB.HTML.FILES)
...
pubapi: $(OUT.DOC.API.PUB).CLEAN do-pubapi
	@echo "Postprocessing Doxygen output."
	@$(MAKE) --no-print-directory -f mk/cs.mak pubapi-postprocess
