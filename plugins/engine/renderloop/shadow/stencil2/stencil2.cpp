...
iRenderBuffer *csStencil2ShadowCacheEntry::GetRenderBuffer (csStringID name)
{
...
csStringID csStencil2ShadowStep::shadow_index_name = csInvalidStringID;
//csStringID csStencil2ShadowStep::shadow_normal_name = csInvalidStringID;
csStringID csStencil2ShadowStep::shadow_vertex_name = csInvalidStringID;
								...
								iShader* shader, size_t shaderTicket, size_t pass)
{
	...
	rmesh.material = 0;
	rmesh.variablecontext = cache_entry->svcontext;
	...
	cache_entry->svcontext->PushVariables (stacks);
	...
	if ((numShadowMeshes = shadowMeshes.Length ()) > 0)
	{
		...
		for (size_t p = 0; p < shadow->GetNumberOfPasses (shaderTicket); p ++) 
		{
			...
			for (int m = 0; m < numShadowMeshes; m++)
			{
				...
				sp->GetMeshObject()->GetObjectModel() ->GetObjectBoundingBox(model_bounding_box);
...
int csStencil2ShadowStep::AddStep (iRenderStep* step)
{
