...
#define CS_DEPTH_TEXTURE_WIDTH 256
#define CS_DEPTH_TEXTURE_HEIGHT 256
...
{
	csRGBpixel *depth_cubemap_data = new csRGBpixel[CS_DEPTH_TEXTURE_WIDTH * CS_DEPTH_TEXTURE_HEIGHT];
	csRef<csImageCubeMapMaker> cubeMaker;
	cubeMaker.AttachNew (new csImageCubeMapMaker ());

	csRef<iImage> cube_side_img = csPtr<iImage> (new csImageMemory (
		CS_DEPTH_TEXTURE_WIDTH, CS_DEPTH_TEXTURE_HEIGHT, depth_cubemap_data, true, 
		CS_IMGFMT_TRUECOLOR | CS_IMGFMT_ALPHA));
...
#undef CS_DEPTH_TEXTURE_WIDTH
#undef CS_DEPTH_TEXTURE_HEIGHT
  ...
  while (nlights-- > 0)
  {
	...
	for (size_t j = 0; j < 6 ; j++)
	{
		...
		csVector3 fll = rview->GetCamera()->GetTransform().This2Other(
			csVector3(-light->GetCutoffDistance(), -light->GetCutoffDistance(), light->GetCutoffDistance() ));
