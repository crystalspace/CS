...
void sys_fatalerror(char *str, HRESULT hRes = S_OK)
{
	...
	if (FAILED(hRes))
	{
		...
		if (dwResult != 0)
		{
			...
			MessageBox (NULL, szMsg, "Fatal Error in Driver2D.dll", MB_OK|MB_TOPMOST);
			delete szMsg;
	...
	MessageBox(NULL, str, "Fatal Error in Driver2D.dll", MB_OK|MB_TOPMOST);
...
EXPORT_CLASS_TABLE (ddraw6)
  EXPORT_CLASS (csGraphics2DDDraw6, "crystalspace.graphics2d.direct3d.dx6",
    "DirectDraw DX6 2D graphics driver for Crystal Space")
  ...
  m_bLocked(false)
{
  ...
  iWin32SystemDriver* piW32Sys* piWin32System = QUERY_INTERFACE (System, iWin32SystemDriver);
  if (!piW32Sys))
...
/* DAN: commented this out until i allow this class to print to iSystem.  
  ...
  if (!FullScreen)
  {
    ...
    {
      sys_fatalerror("Crystal Space requires desktop to be in either 8-bit or 16-bit mode, or to use full screen mode.", ddrval);
      exit(1);
    }
  }
...
bool csGraphics2DDDraw6::SetMouseCursor (csMouseCursorID iShape, TextureMM* iBitmap)
{
  (void)iBitmap;
  switch(iShape)
  {
