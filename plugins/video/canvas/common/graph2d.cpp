...
void csGraphics2D::DrawSprite8 (ITextureHandle *hTex, int sx, int sy,
  int sw, int sh, int tx, int ty, int tw, int th)
{
  int TranspColor; 
  void* bmdata;
  int bw, bh;
  hTex->GetTransparent (TranspColor);
  hTex->GetBitmapDimensions (bw, bh);
  hTex->GetBitmapData (&bmdata);

  unsigned char *bitmap = (unsigned char*)bmdata;
  ...
  if (sw == tw)
  {
    ...
    {
      unsigned char *VRAM = GetPixelAt (sx, sy);
      unsigned char *data = bitmap + (ty >> 16) * bw;
      if (TranspColor >= 0)
      {
        for (int w = sw; w; w--)
        {
          if (*data != (unsigned char)TranspColor)
            *VRAM = *data;
          VRAM++;
          data++;
        } /* endfor */
  ...
  } else
  {
    for (; sh > 0; sh--, ty += dy, sy++)
    {
      ...
      if (TranspColor >= 0)
      {
        for (int w = sw; w; w--)
        {
          unsigned char pixel = data [tx >> 16];
          if (pixel != (unsigned char)TranspColor)
            *VRAM = pixel;
          VRAM++;
          tx += dx;
        } /* endfor */
...
void csGraphics2D::DrawSprite16 (ITextureHandle *hTex, int sx, int sy,
  int sw, int sh, int tx, int ty, int tw, int th)
{
  int TranspColor; 
  void* bmdata;
  ...
  if (sw == tw)
  {
    for (; sh > 0; sh--, ty += dy, sy++)
    {
      unsigned short *VRAM = (unsigned short *)GetPixelAt (sx, sy);
...
void csGraphics2D::DrawSprite32 (ITextureHandle *hTex, int sx, int sy,
  int sw, int sh, int tx, int ty, int tw, int th)
{
  int TranspColor; 
  void* bmdata;
  ...
  if (sw == tw)
  {
    for (; sh > 0; sh--, ty += dy, sy++)
    {
      unsigned long *VRAM = (unsigned long *)GetPixelAt (sx, sy);
      ULong* data = bitmap + (ty >> 16) * bw;
      if (TranspColor >= 0)
      {
        for (int w = sw; w; w--)
        {
          if (*data != (ULong)TranspColor)
            *VRAM = *data;
          VRAM++;
          data++;
        } /* endfor */
  ...
  } else
  {
    for (; sh > 0; sh--, ty += dy, sy++)
    {
      ...
      if (TranspColor >= 0)
      {
        for (int w = sw; w; w--)
        {
          ULong pixel = data [tx >> 16];
          if (pixel != (ULong)TranspColor)
            *VRAM = pixel;
          VRAM++;
          tx += dx;
        } /* endfor */
