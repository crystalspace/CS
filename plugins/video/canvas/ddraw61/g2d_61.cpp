...
EXPORT_CLASS_TABLE (ddraw61)
  EXPORT_CLASS_DEP (csGraphics2DDDraw6, "crystalspace.graphics2d.direct3d.dx61",
    "DirectDraw DX6.1 2D graphics driver for Crystal Space", "crystalspace.font.server.")
...
csGraphics2DDDraw6::~csGraphics2DDDraw6 ()
{
  Close ();
}
...
bool csGraphics2DDDraw6::Open (const char *Title)
{
  ...
  if ((ddrval = lpDD->QueryInterface (IID_IDirectDraw4, (LPVOID *)&m_lpDD4)) != DD_OK)
  {
    InitFail (ddrval, "Cannot get DirectDraw4 interface (Code: %08lx)\n");
    return false;
  }
...
void csGraphics2DDDraw6::Refresh (RECT &rect)
{
  if ((FullScreen && m_bDoubleBuffer)
...
bool csGraphics2DDDraw6::PerformExtension (const char *iCommand, ...)
{
  va_list args;
...
//-----------------------------------------------------------------------------
HRESULT csGraphics2DDDraw6::ReleaseAllObjects ()
{
  if (m_lpDD4)
  {
    if (m_lpddsBack)
    ...
    m_lpDD4->SetCooperativeLevel (m_hWnd, DDSCL_NORMAL);
  }
  return DD_OK;
}

HRESULT csGraphics2DDDraw6::InitSurfaces ()
{
  HRESULT ddrval;
  DDSURFACEDESC2 ddsd;
  DDSCAPS2 ddscaps;
  ...
  ddrval = m_lpDD4->SetCooperativeLevel (m_hWnd, FullScreen ?
  ...
  {
    // Set FS video mode
    ddrval = m_lpDD4->SetDisplayMode (Width, Height, Depth, 0, 0);
    if (ddrval != DD_OK)
  ...
  if (!FullScreen || !m_bDoubleBuffer)
  {
    ...
    if ((ddrval = m_lpDD4->CreateSurface (&ddsd, &m_lpddsPrimary, NULL)) != DD_OK)
    ...
    if ((ddrval = m_lpDD4->CreateSurface (&ddsd, &m_lpddsBack, NULL)) != DD_OK)
    {
...
//-----------------------------------------------------------------------------
HRESULT csGraphics2DDDraw6::ChangeCoopLevel ()
{
  HRESULT hRet;
...
//-----------------------------------------------------------------------------
HRESULT csGraphics2DDDraw6::InitFail (HRESULT ddrval, LPCTSTR szError)
{
  ReleaseAllObjects ();
  if (m_lpDD4)
    m_lpDD4->RestoreDisplayMode ();
...
void csGraphics2DDDraw6::ClearSystemPalette ()
{
...
long FAR PASCAL csGraphics2DDDraw6::WindowProc (HWND hWnd, UINT message,
  WPARAM wParam, LPARAM lParam)
{
  csGraphics2DDDraw6 *This = (csGraphics2DDDraw6 *)GetWindowLong (hWnd, GWL_USERDATA);
  switch (message)
  {
    case WM_PAINT:
...
void csGraphics2DDDraw6::SetModeSwitchCallback (void (*Callback) (void *), void *Data)
{
