...
{
    // kill all the fonts data we shoved into openGL, if any
    if (Font_Offsets)
    {
      // cycle through all loaded fonts
      for (int index=0; index < Font_Count; index++)
      {
        glDeleteLists(Font_Offsets[index],128);
      }
      CHKB(delete [] Font_Offsets);
    }
}
...
void csGraphics2DOpenGLFontServer::BuildFont(FontDef &newfont)
{
    ...
    // now shove all the raster data at openGL...
...
void csGraphics2DOpenGLFontServer::WriteCharacters(char *writeme, int fontnumber)
{
    ...
    // get old state (list offset) back
    glPopAttrib();
}
