...
#define DLL_NAME "glx2d.so"

static unsigned int gRefCount = 0;
static DllRegisterData gRegData =
{
  &CLSID_GLXGraphics2D,
  "crystalspace.graphics2d.glx",
  "Crystal Space 2D driver for OpenGL using GL/X"
};

csGraphics2DGLXFactory gG2DGLXFactory;

#ifdef CS_STATIC_LINKED

void Glx2DRegister ()
{
  gRegData.pClass = &gG2DGLXFactory;
  csRegisterServer (&gRegData);
}

void Glx2DUnregister ()
{
...
{
  if (rclsid == CLSID_GLXGraphics2D)
    return gG2DGLXFactory.QueryInterface (riid, ppv);
...
IMPLEMENT_UNKNOWN_NODELETE (csGraphics2DGLXFactory)

BEGIN_INTERFACE_TABLE (csGraphics2DGLXFactory)
  IMPLEMENTS_INTERFACE (IGraphics2DFactory)
END_INTERFACE_TABLE ()

STDMETHODIMP csGraphics2DGLXFactory::CreateInstance (REFIID riid, ISystem * piSystem, void **ppv)
{
  if (!piSystem)
  {
    *ppv = 0;
  ...
  csGraphics2DGLX *pNew = new csGraphics2DGLX (piSystem);
...
STDMETHODIMP csGraphics2DGLXFactory::LockServer (COMBOOL bLock)
{
  if (bLock)
    gRefCount++;
  else
