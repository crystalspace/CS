...
csTextureMMDirect3D::csTextureMMDirect3D (IImageFile* image) : csTextureMM (image)
{
  hicolorcache = NULL;
  in_memory = false;
}
...
void csTextureMMDirect3D::convert_to_internal (csTextureManager* tex,
...
void csTextureMMDirect3D::remap_texture (csTextureManager* new_palette)
{
  if (!ifile) return;
