...
csTextureMMOpenGL::csTextureMMOpenGL (IImageFile* image) : csTextureMM (image)
{
  hicolorcache = NULL;
  in_memory = false;
}
...
void csTextureMMOpenGL::convert_to_internal (csTextureManager* tex,
...
void csTextureMMOpenGL::remap_texture (csTextureManager* new_palette)
{
  if (!ifile) return;

  if (for_2d ())
    if (((csTextureManagerOpenGL*)new_palette)->get_display_depth () == 16) remap_texture_16 (new_palette);
...
void csTextureMMOpenGL::remap_palette_24bit (csTextureManager*)
{
  compute_color_usage ();
  if (!usage) return;
...
void csTextureMMOpenGL::remap_texture_16 (csTextureManager* new_palette)
{
  int w, h, r, g, b;
  ifile->GetWidth (w);
  ifile->GetHeight (h);
...
void csTextureMMOpenGL::remap_texture_32 (csTextureManager* new_palette)
{
  int w, h, r, g, b;
  ifile->GetWidth (w);
  ifile->GetHeight (h);
