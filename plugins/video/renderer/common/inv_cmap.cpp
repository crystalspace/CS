	 ...
	 unsigned char *rgbmap)
{
	 ...
	 */
	rcenter = colormap[0][cindex] >> rnbits;
	gcenter = colormap[1][cindex] >> gnbits;
	bcenter = colormap[2][cindex] >> bnbits;

	rdist = colormap[0][cindex] - (rcenter * rx + rx/2);
	gdist = colormap[1][cindex] - (gcenter * gx + gx/2);
	cdist = colormap[2][cindex] - (bcenter * bx + bx/2);
	cdist = rdist*rdist + gdist*gdist + cdist*cdist;

	crinc = 2 * ((rcenter + 1) * rxsqr - (colormap[0][cindex] * rx));
	cginc = 2 * ((gcenter + 1) * gxsqr - (colormap[1][cindex] * gx));
	cbinc = 2 * ((bcenter + 1) * bxsqr - (colormap[2][cindex] * bx));
...
maxfill( unsigned long *buffer, long rside, long gside, long bside )
{
    ...
    for ( i = rside * gside * bside, bp = buffer;
	  i > 0;
	  i--, bp++ )
	*bp = maxv;
}
