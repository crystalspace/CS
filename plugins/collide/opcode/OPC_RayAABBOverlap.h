...
inline_ bool RayCollider::SegmentAABBOverlap(const Point& center, const Point& extents)
{
	...
	float Dx = mData2.x - center.x;		if(fabs(Dx) > extents.x + mFDir.x)	return false;
	float Dy = mData2.y - center.y;		if(fabs(Dy) > extents.y + mFDir.y)	return false;
	float Dz = mData2.z - center.z;		if(fabs(Dz) > extents.z + mFDir.z)	return false;

	float f;
	f = mData.y * Dz - mData.z * Dy;	if(fabs(f) > extents.y*mFDir.z + extents.z*mFDir.y)	return false;
	f = mData.z * Dx - mData.x * Dz;	if(fabs(f) > extents.x*mFDir.z + extents.z*mFDir.x)	return false;
	f = mData.x * Dy - mData.y * Dx;	if(fabs(f) > extents.x*mFDir.y + extents.y*mFDir.x)	return false;
...
//	float Dx = mOrigin.x - center.x;	if(fabs(Dx) > extents.x && Dx*mDir.x>=0.0f)	return FALSE;
//	float Dy = mOrigin.y - center.y;	if(fabs(Dy) > extents.y && Dy*mDir.y>=0.0f)	return FALSE;
//	float Dz = mOrigin.z - center.z;	if(fabs(Dz) > extents.z && Dz*mDir.z>=0.0f)	return FALSE;
...
//	float Dz = mOrigin.z - center.z;	if(GREATER(Dz, extents.z) && ((SIR(Dz)-1)^SIR(mDir.z))>=0.0f)	return FALSE;
	...
	float f;
	f = mDir.y * Dz - mDir.z * Dy;		if(fabs(f) > extents.y*mFDir.z + extents.z*mFDir.y)	return FALSE;
	f = mDir.z * Dx - mDir.x * Dz;		if(fabs(f) > extents.x*mFDir.z + extents.z*mFDir.x)	return FALSE;
	f = mDir.x * Dy - mDir.y * Dx;		if(fabs(f) > extents.x*mFDir.y + extents.y*mFDir.x)	return FALSE;
