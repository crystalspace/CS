...
csSkeletonBone::csSkeletonBone (csSkeleton *skeleton, csSkeletonBoneFactory *factory_bone)
{
  ...
  cb = csPtr<iSkeletonBoneUpdateCallback>(new csSkeletonBoneDefaultUpdateCallback());
...
void csSkeletonBone::SetParent (iSkeletonBone *par)
{
  ...
  {
    int child_index = parent->FindChildIndex((iSkeletonBone *)this);
    if (child_index > -1)
    {
...
int csSkeletonBone::FindChildIndex (iSkeletonBone *child)
{
  for (size_t i = 0; i < bones.Length () ; i++)
  {
...
csSkeletonBoneFactory::csSkeletonBoneFactory (csSkeletonFactory *skeleton_factory)
{
  ...
  ragdoll_info = csPtr<iSkeletonBoneRagdollInfo>(new csSkeletonBoneRagdollInfo(this));
}
...
int csSkeletonBoneFactory::FindChildIndex (iSkeletonBoneFactory *child)
{
  for (size_t i = 0; i < bones.Length () ; i++)
  {
...
void csSkeletonBoneFactory::SetParent (iSkeletonBoneFactory *par)
{
  if (parent && (par != parent))
  {
    int child_index = parent->FindChildIndex((iSkeletonBoneFactory *)this);
    if (child_index > -1)
    {
      parent->GetBones().DeleteIndexFast(child_index);
    }
...
iSkeletonScriptKeyFrame *csSkeletonScript::CreateFrame(const char* name)
{
  csRef<csSkeletonScriptKeyFrame> key_frame = 
    (csPtr<csSkeletonScriptKeyFrame>)(new csSkeletonScriptKeyFrame (name));
...
{
  for (int i = 0; i < frame->GetTransformsCount() ; i++)
  {
  csReversibleTransform & transform = frame->GetTransform(i);
  bone_transform_data *bone_transform = GetBoneTransform ((csSkeletonBoneFactory *)frame->GetBone(i));
  ...
  else
  {
    ...
    //if (current_frame == 0)
    {
      //m.final_position = bone_transform->pos;
      //m.quat = bone_transform->quat;
    }
    //else
    {
...
bone_transform_data *csSkeletonRunnable::GetBoneTransform(csSkeletonBoneFactory *bone_fact)
{
  ...
  if (!b_tr) 
  {
  int index = ((csSkeletonFactory *)skeleton->GetFactory())->FindBoneIndex(bone_fact);
...
csSkeleton::csSkeleton(csSkeletonFactory* fact)
{
  ...
  for (size_t i = 0; i < fact_bones.Length(); i++ )
  {
    csRef<csSkeletonBone> bone = 
      (csPtr<csSkeletonBone>)(new csSkeletonBone (this, fact_bones[i]));
  ...
  for (size_t i = 0; i < bones.Length(); i++ )
  {
    ...
    if (fact_parent_bone)
    {
    int index = fact->FindBoneIndex((csSkeletonBoneFactory *)fact_parent_bone);
    if (index > -1)
    {
  ...
  for (size_t i = 0; i < fact_sockets.Length(); i++ )
  {
    ...
    int index = fact->FindBoneIndex((csSkeletonBoneFactory *)fact_sockets[i]->GetBone());
    if (index > -1)
    {
...
int csSkeleton::FindBoneIndex (const char* bonename)
{
  size_t i;
  for (i = 0 ; i < bones.Length () ; i++)
  {
...
iSkeletonBoneFactory *csSkeletonFactory::CreateBone(const char *name)
{
  csRef<csSkeletonBoneFactory> bone = 
    (csPtr<csSkeletonBoneFactory>)(new csSkeletonBoneFactory(this));
  bone->SetName(name);
  bones.Push(bone);
  return bone; 
}
...
int csSkeletonFactory::FindBoneIndex (const char* bonename)
{
  size_t i;
  for (i = 0 ; i < bones.Length () ; i++)
  {
...
int csSkeletonFactory::FindBoneIndex (csSkeletonBoneFactory *bone) const
{
  size_t i;
  for (i = 0 ; i < bones.Length () ; i++)
    if (bones[i] == bone)
...
iSkeletonScript *csSkeletonFactory::CreateScript(const char *name)
{
  csRef<csSkeletonScript> script = 
    (csPtr<csSkeletonScript>)(new csSkeletonScript (name));
  scripts.Push(script);
  return script;
}
...
iSkeletonSocketFactory *csSkeletonFactory::CreateSocket(const char *name, iSkeletonBoneFactory *bone)
{
  csRef<csSkeletonSocketFactory> socket = 
    (csPtr<csSkeletonSocketFactory>)(new csSkeletonSocketFactory (name, bone));
  sockets.Push(socket);
  return socket;
}
...
int csSkeletonFactory::GetSocketsCount()
{
...
iSkeletonFactory* csSkeletonGraveyard::CreateFactory(const char *name)
{
  csRef<csSkeletonFactory> fact = 
    (csPtr<csSkeletonFactory>)(new csSkeletonFactory (this, object_reg));
  fact->SetName(name);
  factories.Push(fact);
  return  fact;
}
