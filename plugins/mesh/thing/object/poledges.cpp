...
{
  iterator = new csGlobalHashIterator (&edges);
  if (iterator->HasNext ())
    current = (csPolEdge *) (iterator->Next ());
  else
    current = 0;
  ...
  edges(25247)  // Some prime number
{
  ...
  for (i = 0; i < num_polygons; i++)
  {
    ...
    int pi_count = p->GetStaticPoly ()->GetVertexCount ();
    int* pi = p->GetStaticPoly ()->GetVertexIndices ();
    j1 = pi_count - 1;
...
{
  csGlobalHashIterator iterator (&edges);
