...
///
///   the idea is that at startup each cloth meshobject gets its initial state from
///   its factory, and when it needs to reset it (btw, which interface could allow me
...
{
	if (scfParent->factory_triangles)   { delete [] scfParent->factory_triangles; };
...
{ 
	...
	int k;
     ...
     for (i=0;i<nverts;i++) {
		 ...
		 scfParent->factory_texels[i].Set(xindex/2.1f , yindex/2.1f );
   ...
   for (i=0;i<Xsize;i++) {
	for (j=0;j<Ysize;j++) {
	   ...
	   scfParent->factory_triangles[k].a=i + (Xsize + 1)*j;   // ahh lot of funs with triangles
