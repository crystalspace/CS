...
static void aws_ErrorReporter (JSContext *cx, const char *message, 
...
static JSBool Exec (JSContext *cx, JSObject *obj, uintN argc, jsval *argv, 
                    jsval *rval)	
{
...
static JSBool GarbageCollect (JSContext *cx, JSObject *obj, uintN argc, 
...
static JSBool GarbageCollectSmart (JSContext *cx, JSObject *obj, uintN argc, 
                                   jsval *argv, jsval *rval)	
{
...
static JSBool CreateTimer (JSContext *cx, JSObject *obj, uintN argc, 
                           jsval *argv, jsval *rval)	
{
  int32 ticks;
...
/// Get the rendering buffer width.
static JSBool GetWidth (JSContext *cx, JSObject *obj, uintN argc, jsval *argv, 
                        jsval *rval)	
{			
...
/// Get the rendering buffer height.
static JSBool GetHeight (JSContext *cx, JSObject *obj, uintN argc, jsval *argv, 
                         jsval *rval)	
{
...
void scriptManager::Initialize (iObjectRegistry *obj_reg)
{
  csString msg;
...
JSObject *scriptManager::CreateObject (const char *name, JSClass *clasp)
{
...
void scriptManager::Exec (const csString &filename)
{
...

  {
    ...
    sc=JS_CompileScript (cx, global,
      buf->GetData (), buf->GetSize (),
      filename.GetData (), 1);

    if(sc!=0)
    {
      if (JS_ExecuteScript (ScriptMgr ()->GetContext (), 
            ScriptMgr ()->GetGlobalObject (), sc, &tmp)==JS_FALSE)
      {						  					
