...
namespace autom
{
  ...
  /**
   * The reference counted pointer class that lets keila objects go away when
  ...
  public:
    ...
    virtual bool parseObject(std::string::iterator &pos,
    	const std::string::iterator &end)=0;
  };
  ...
  public:  
    ...
    /**
     * Assumes that the string holds the text representation of a list, and
     * parses it.
     */
    list(std::string &s);        
    ...
    /**
     * Returns a keeper to the object at index i of the list.  If I is out of
     * bounds, then a keeper to NIL is returned.
     */
    keeper at(size_t i);
  ...
  public:  
    ...
    reference(const reference& s):iObject(), object(s, T_REFERENCE),
    	value(s.value), fn(s.fn) {}  
