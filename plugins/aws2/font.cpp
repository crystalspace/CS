...
/// The prototype object for fonts.
static JSObject *font_proto_object=0;

/// The constructor for color objects.
static JSBool Font (JSContext *cx, JSObject *obj, uintN argc, jsval *argv, 
                    jsval *rval)	
{
  csString msg;
  ...
  else
  {
    ...
    csRef<iFont> *fo = new csRef<iFont>(
      AwsMgr ()->G2D ()->GetFontServer ()->LoadFont (font_name.GetData (), 
      (float)size));
...
/// Gets the baseline to baseline height of the font.
static JSBool GetTextHeight (JSContext *cx, JSObject *obj, uintN argc, 
                             jsval *argv, jsval *rval)	
{
  csRef<iFont> *fo = (csRef<iFont> *)JS_GetPrivate (cx, obj);

  if (fo)
    *rval = INT_TO_JSVAL((*fo)->GetTextHeight ());

  return JS_TRUE;
}

/**
 * Determine how many characters from this string can be written without 
 * exceeding given width (in pixels).  
 */
static JSBool GetLength (JSContext *cx, JSObject *obj, uintN argc, jsval *argv, 
                         jsval *rval)	
{
  csRef<iFont> *fo = (csRef<iFont> *)JS_GetPrivate (cx, obj);		
...
/// Used to return the dimensions a string of text in some font.
...
bool IsFontObject (JSObject *obj)
{
  return JS_InstanceOf (ScriptMgr ()->GetContext (), obj, 
    &font_object_class, NULL) == JS_TRUE;
}
