 ...
 * 
 * The widget object is used as a base for all user-interface objects.  
 */
...
namespace aws
{
  ...
  */
  ...
  {    
    ...
    /// The list of children that this widget has. Children draw *after* the parent.
    csArray<widget *> children;

    /** 
     * The list of <i>sibling</i> widgets that are docked to this widget.  
     * Whenever a sibling gets a move or resize request, it updates 
     * all of the widgets in it's docked list. That way they end up 
     * moving/resizing together.
     */
    widget *docked[4];

    /** 
     * The list of <i>sibling</i> widgets that this widget is docked to.  
     * If this widget receives a move or resize request, the resize or move 
     * will be reflected to the appropriate widget.
     */
    widget *docked_to[4];

    /// A set of bits that tell us what sides of our parent we want to stick to.
  ...
  public:
    ...
    widget *NorthMost () 
    { 
      if (docked_to[W_DOCK_NORTH]) 
        return docked_to[W_DOCK_NORTH]->NorthMost(); 
      else 
        return this; 
    }
    ...
    * of this widget. */
    ...
    /// Adds a child widget into this widget.
    ...
    //// Adds a docking widget to this widget.
    ...
    /// Adjusts widgets docked to this widget.
    void AdjustDocked (int which=-1)
    {
      ...
      {	  	 	
        ...
        {
        case W_DOCK_NORTH:		  		
          w->Bounds ().SetPos (Bounds ().xmin, 
            Bounds ().ymin-w->Bounds ().Height()-margins[W_DOCK_SOUTH]);
        ...
        case W_DOCK_EAST:
          w->Bounds ().SetPos (Bounds ().xmax+1+margins[W_DOCK_WEST], 
            Bounds ().ymin);
        ...
        case W_DOCK_WEST:
          w->Bounds ().SetPos (Bounds ().xmin-w->Bounds ().Width ()-
            margins[W_DOCK_EAST], Bounds ().ymin);
          w->Bounds ().SetSize (w->Bounds ().Width (), Bounds().Height ());
          w->Invalidate ();		  		
          break;			  	
        }
    ...
    /// Reflects the movement to the appropriate widget. 
    ...
    /**
     * Returns a pointer to the widget that contains this point.  
     * Either this widget or a child will be returned on success.  
    ...
    /** 
     * Adjusts our frame to our parent's frame, accounting for the sticky bits. 
     * There are two sets of sticky bits.  The first set indicates whether the 
     * sticky side is on or off.  If it's off, nothing happens.  If it's on, 
     * the default behavior is to stick that side of the frame the the same 
     * side of the parent frame.  This will generally cause that side to move 
     * in and out while the other side remains static.
     * The net result is that the frame will stretch and shrink.  In many cases
     * this is what you want. In other cases you might want the frame to stick 
     * to one side of the parent frame, but retain it's shape.  In that case, 
     * the second set of sticky bits comes into play. 
     * If the second set of bits (the no-stretch set) has the bit set, this 
     * function makes sure that the widget retains it's shape. 
     */
    ...
    /// Returns true if the widget has been invalidated, false otherwise.
