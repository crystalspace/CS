...
/// The prototype object for widgets. 
...
/// Forwards a widget GetProperty call.
static JSBool widget_getProperty (JSContext *cx, JSObject *obj, jsval id, 
                                  jsval *vp)
{
...
/// Forwards a widget GetProperty call.
static JSBool widget_get_staticProperty (JSContext *cx, JSObject *obj, jsval id, 
                                         jsval *vp)
{
...
/// Forwards a widget SetProperty call.
static JSBool widget_setProperty (JSContext *cx, JSObject *obj, jsval id, 
                                  jsval *vp)
{
  aws::widget *wo = (aws::widget *)JS_GetPrivate (cx, obj);
...
/// The constructor for widget objects.
static JSBool Widget (JSContext *cx, JSObject *obj, uintN argc, jsval *argv, 
                      jsval *rval)	
{
...
/// Move a widget by (x,y) 
static JSBool Move (JSContext *cx, JSObject *obj, uintN argc, jsval *argv, 
                    jsval *rval)	
{
...
/// Move a widget to (x,y)
static JSBool MoveTo (JSContext *cx, JSObject *obj, uintN argc, jsval *argv, 
                      jsval *rval)	
{
  if (argc<2) return JS_FALSE;
...
/// Resize a widget by (w,h)
static JSBool Resize (JSContext *cx, JSObject *obj, uintN argc, jsval *argv, 
                      jsval *rval)	
{
  if (argc<2) return JS_FALSE;
...
/// Resize a widget to (w,h)
static JSBool ResizeTo (JSContext *cx, JSObject *obj, uintN argc, jsval *argv, 
                        jsval *rval)	
{
  if (argc<2) return JS_FALSE;
...
/// Invalidate a widget.
static JSBool Invalidate (JSContext *cx, JSObject *obj, uintN argc, 
...
/// Gets the pen for a widget.
static JSBool GetPen (JSContext *cx, JSObject *obj, uintN argc, jsval *argv, 
                      jsval *rval)	
{		
  aws::widget *wo = (aws::widget *)JS_GetPrivate (cx, obj);
...
/// Sets the pen for a widget.
static JSBool SetPen (JSContext *cx, JSObject *obj, uintN argc, jsval *argv, 
                      jsval *rval)	
{		
  aws::widget *wo = (aws::widget *)JS_GetPrivate (cx, obj);
...
/// Adds a child object.
static JSBool AddChild (JSContext *cx, JSObject *obj, uintN argc, jsval *argv, 
                        jsval *rval)	
{		
  aws::widget *wo = (aws::widget *)JS_GetPrivate (cx, obj);
...
static JSBool RemoveChild (JSContext *cx, JSObject *obj, uintN argc, 
                           jsval *argv, jsval *rval)	
{		
  aws::widget *wo = (aws::widget *)JS_GetPrivate (cx, obj);
  ...
  {
    JS_ReportError (cx, 
      "Trying to remove an object that is not a Widget as a child is prohibited.");
    return JS_FALSE;	
  } 
...
/// Adds a child object.
static JSBool Dock(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, 
                   jsval *rval)	
{	
  int32 where;	
  aws::widget *wo = (aws::widget *)JS_GetPrivate (cx, obj);
  ...
  {
    JS_ReportError (cx, 
      "Trying to dock an object that is not a Widget is prohibited.");
    return JS_FALSE;	
  } 
...
/// Sets the frame anchor flags for a widget.
static JSBool SetFrameAnchor (JSContext *cx, JSObject *obj, uintN argc, 
                              jsval *argv, jsval *rval)	
{	
  int32 where;	
  aws::widget *wo = (aws::widget *)JS_GetPrivate (cx, obj);
...
/// Clears a frame anchor.
static JSBool ClearFrameAnchor (JSContext *cx, JSObject *obj, uintN argc, 
                                jsval *argv, jsval *rval)	
{	
  int32 where;	
  aws::widget *wo = (aws::widget *)JS_GetPrivate (cx, obj);
...
/// Sets the margin.
static JSBool SetMargin (JSContext *cx, JSObject *obj, uintN argc, 
                         jsval *argv, jsval *rval)	
{	
  int32 where, size;	
  aws::widget *wo = (aws::widget *)JS_GetPrivate (cx, obj);
...
/// Capture the mouse to this widget.
static JSBool CaptureMouse (JSContext *cx, JSObject *obj, uintN argc, 
                            jsval *argv, jsval *rval)	
{		
  aws::widget *wo = (aws::widget *)JS_GetPrivate (cx, obj);
...
/// Release the mouse from this widget.
static JSBool ReleaseMouse (JSContext *cx, JSObject *obj, uintN argc, 
                            jsval *argv, jsval *rval)	
{		
  //aws::widget *wo = (aws::widget *)JS_GetPrivate(cx, obj);
...
/// Capture the mouse to this widget.
static JSBool Show (JSContext *cx, JSObject *obj, uintN argc, jsval *argv, 
                    jsval *rval)	
{		
  aws::widget *wo = (aws::widget *)JS_GetPrivate (cx, obj);
...
/// Release the mouse from this widget.
static JSBool Hide (JSContext *cx, JSObject *obj, uintN argc, jsval *argv, 
                    jsval *rval)	
{		
  aws::widget *wo = (aws::widget *)JS_GetPrivate (cx, obj);
...
/// Broadcast an event to all children (by calling the given function).
static JSBool Broadcast (JSContext *cx, JSObject *obj, uintN argc, jsval *argv, 
                         jsval *rval)	
{		
  aws::widget *wo = (aws::widget *)JS_GetPrivate (cx, obj);
...
namespace aws
{
  ...
  bool widget::SetProperty (JSContext *cx, JSObject *obj, jsval id, jsval *vp)
  {
    ...
    {	
      ...
      case WIDGET_WIDTH:  
        { 
          int32 w; 
          JS_ValueToInt32 (cx, *vp, &w);  
          Bounds ().SetSize (w, Bounds ().Height ()); 
        } 
        break;
      case WIDGET_HEIGHT: 
        { 
          int32 h; 
          JS_ValueToInt32 (cx, *vp, &h);  
          Bounds ().SetSize (Bounds ().Width (), h); 
        } 
        break;
  ...
  bool widget::GetProperty (JSContext *cx, JSObject *obj, jsval id, jsval *vp)
  {
    ...
    {	
      ...
      case WIDGET_PARENT: 
        *vp = (parent ? OBJECT_TO_JSVAL(parent->WidgetObject ()) : JSVAL_NULL); 
        break;
      default: return false;
      }  
  ...
  {    
    ...
    {	
      ...
      {			
        ...
        JS_CallFunctionName (ScriptMgr ()->GetContext (), w_object, "onDraw", 
          1, argv, &rval);
  ...
  bool widget::HandleEvent (iEvent &Event)
  {
    ...
    {
      if (JS_GetProperty (ScriptMgr ()->GetContext (), WidgetObject (), 
        "onMouseMove", &func_val)==JS_TRUE && func_val!=JSVAL_VOID)
      {
        jsval args[3];
        ...
        JS_CallFunctionValue (ScriptMgr ()->GetContext (), WidgetObject (), 
          func_val, 5, args, &rv);
    ...
    {
      if (JS_GetProperty (ScriptMgr ()->GetContext (), WidgetObject (), 
        "onMouseUp", &func_val)==JS_TRUE && func_val!=JSVAL_VOID)
      {
        jsval args[3];
        int x = csMouseEventHelper::GetX (&Event),
          y = csMouseEventHelper::GetY (&Event),
    ...
    {
      if (JS_GetProperty (ScriptMgr ()->GetContext (), WidgetObject (), 
        "onMouseDown", &func_val)==JS_TRUE && func_val!=JSVAL_VOID)
      {
        jsval args[3];
        int x = csMouseEventHelper::GetX (&Event),
          y = csMouseEventHelper::GetY (&Event),
    ...
    {
      if (JS_GetProperty (ScriptMgr ()->GetContext (), WidgetObject (), 
        "onMouseClick", &func_val)==JS_TRUE && func_val!=JSVAL_VOID)
      {
        jsval args[3];
        int x = csMouseEventHelper::GetX (&Event),
          y = csMouseEventHelper::GetY (&Event);
    ...
    {
      if (JS_GetProperty (ScriptMgr ()->GetContext (), WidgetObject (), 
        "onMouseEnter", &func_val)==JS_TRUE && func_val!=JSVAL_VOID)
      {			
        JS_CallFunctionValue (ScriptMgr ()->GetContext (), WidgetObject (), 
          func_val, 0, NULL, &rv);
    ...
    {
      if (JS_GetProperty (ScriptMgr ()->GetContext(), WidgetObject (), 
        "onMouseExit", &func_val)==JS_TRUE && func_val!=JSVAL_VOID)
      {			
        JS_CallFunctionValue (ScriptMgr ()->GetContext(), WidgetObject (), 
          func_val, 0, NULL, &rv);
    ...
    {
      if (JS_GetProperty (ScriptMgr ()->GetContext (), WidgetObject (), 
        "onGainFocus", &func_val)==JS_TRUE && func_val!=JSVAL_VOID)
      {			
        JS_CallFunctionValue (ScriptMgr ()->GetContext (), WidgetObject (), 
          func_val, 0, NULL, &rv);
    ...
    {
      if (JS_GetProperty (ScriptMgr ()->GetContext (), WidgetObject (), 
        "onLoseFocus", &func_val)==JS_TRUE && func_val!=JSVAL_VOID)
      {			
        JS_CallFunctionValue (ScriptMgr ()->GetContext (), WidgetObject (), 
          func_val, 0, NULL, &rv);
    ...
    {
      if (JS_GetProperty (ScriptMgr ()->GetContext (), WidgetObject (), 
        "onFrameStart", &func_val)==JS_TRUE && func_val!=JSVAL_VOID)
      {			
        JS_CallFunctionValue (ScriptMgr ()->GetContext (), WidgetObject (), 
          func_val, 0, NULL, &rv);
  ...
  void widget::Broadcast (JSContext *cx, JSObject *obj, uintN argc, 
    jsval *argv, jsval *rval)
  {
    if (argc==0) return;
    ...
    for(size_t i=0; i<children.Length (); ++i)
    {
      ...
      if (JS_GetProperty (cx, children[i]->WidgetObject (), func_name, 
        &func_val)==JS_TRUE && func_val!=JSVAL_VOID)
      {			
        JS_CallFunctionValue (cx, children[i]->WidgetObject (), func_val, 
          argc-1, &argv[1], &rv);			
      }				
    }
  ...
  void widget::Signal (const char *func_name)
  {
    ...
    if (JS_GetProperty (ScriptMgr ()->GetContext (), WidgetObject (), 
      func_name, &func_val)==JS_TRUE && func_val!=JSVAL_VOID)
    {
      JS_CallFunctionValue (ScriptMgr ()->GetContext (), WidgetObject (), 
        func_val, 0, NULL, &rv);			
    }
