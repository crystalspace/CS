...
/** @brief The prototype object for textures. */
static JSObject *texture_proto_object=0;

/** @brief The constructor for color objects. */
static JSBool
Texture(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)	
{
	csString msg;
	...
	if (JSVAL_IS_OBJECT(argv[0]))
	{
		JSObject *texture_object = JSVAL_TO_OBJECT(argv[0]);
		 ...
		 {		
			// Copy constructor.	
			csRef<iTextureHandle> *fo = (csRef<iTextureHandle> *)JS_GetPrivate(cx, texture_object);		
	...
	else if (JSVAL_IS_INT(argv[0]))
	{
		...
		csRef<iTextureHandle> *to = new csRef<iTextureHandle>(AwsMgr()->G3D()->GetTextureManager()->RegisterTexture(img, CS_TEXTURE_2D | CS_TEXTURE_3D));
		if (!to)
		{
	...
	else
	{
		...
		csRef<iImageIO> loader = CS_QUERY_REGISTRY (AwsMgr()->GetObjectRegistry(), iImageIO);
		...
		if (!buf)
		{
			msg.Format("Texture: Could not load '%s' because the file could not be read.", texture_name.GetDataSafe());
			ScriptCon()->Message(msg);	
		...
		if (!ifile)
		{
			msg.Format("Texture: Could not load '%s', the image format is unknown.", texture_name.GetDataSafe());
			ScriptCon()->Message(msg);	
			
			return JS_FALSE;
		}
		
		csRef<iTextureHandle> *to = new csRef<iTextureHandle>(txtmgr->RegisterTexture(ifile, CS_TEXTURE_2D | CS_TEXTURE_3D));
		if (!to)
		{
			msg.Format("Texture: Could not register '%s' with the texture manager.", texture_name.GetDataSafe());
			ScriptCon()->Message(msg);	
...
/** @brief Used to return the dimensions a texture. */
...
/** @brief Loads a texture object from disk. */
static JSBool
...
bool 
IsTextureObject(JSObject *obj)
{
	return JS_InstanceOf(ScriptMgr()->GetContext(), obj, &texture_object_class, NULL) == JS_TRUE;
}
