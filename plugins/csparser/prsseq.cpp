...
#define PARTYPE_POLYGON 6
    ...
    else
    {
      ...
      case PARTYPE_TRIGGER:
        ...
        if (!value)
        {
          ...
          AddToCollection (ldr_context, trig->QueryObject ());
          value = trig;
        }
        break;
    ...
    iDocumentNode* node)
  {
    ...
    if (!trigger)
    {
      ...
      AddToCollection (ldr_context, trigger->QueryObject ());
    }
      ...
      case XMLTOKEN_SEQUENCE:
        {
          ...
          AddToCollection (ldr_context, sequence->QueryObject ());
        }
        break;
      default:
        SyntaxService->ReportBadToken (child);
  ...
  csPtr<iParameterESM> csThreadedLoader::ResolveOperationParameter (
    iLoaderContext* ldr_context,
    iDocumentNode* opnode,
    int partypeidx, const char* partype, const char* seqname,
    iEngineSequenceParameters* base_params)
  ...
  csPtr<iEngineSequenceParameters> csThreadedLoader::CreateSequenceParameters (
    iLoaderContext* ldr_context,
    iSequenceWrapper* sequence, iDocumentNode* node,
    const char* parenttype, const char* parentname, bool& error)
  {
  ...
  iSequenceTrigger* csThreadedLoader::LoadTrigger (iLoaderContext* ldr_context,
    iDocumentNode* node)
  {
    const char* trigname = node->GetAttributeValue ("name");
    // LoadTriggers() already checked for the presence of the engine
  ...
  iSequenceWrapper* csThreadedLoader::LoadSequence (iLoaderContext* ldr_context,
    iDocumentNode* node)
  {
    const char* seqname = node->GetAttributeValue ("name");
    // LoadSequences() already checked for the presence of the engine
      ...
      case XMLTOKEN_SEQUENCE:
        {
          iSequenceWrapper* sequence = CreateSequence (child);
          if (!sequence) return false;
          ldr_context->AddToCollection(sequence->QueryObject ());
        }
        break;
      default:
        SyntaxService->ReportBadToken (child);
  ...
  iSharedVariable *csThreadedLoader::FindSharedVariable(const char *colvar,
    int verify_type )
  {
    iSharedVariable *found = Engine->GetVariableList ()->FindByName (colvar);
    if (found)
    {
      if (verify_type == iSharedVariable::SV_UNKNOWN ||
        found->GetType() == verify_type)
