...
{
  &CLSID_NullSoundDriver,
  "crystalspace.sound.driver.null",
  "Crystal Space Null Sound Driver"
};

#ifdef CS_STATIC_LINKED

void SoundDriverNullRegister ()
{
  static csSoundDriverNullFactory gNullFactory;
  gRegData.pClass = &gNullFactory;
  csRegisterServer (&gRegData);
}

void SoundDriverNullUnregister ()
{
...
{
  static csSoundDriverNullFactory gNullFactory;
  if (rclsid == CLSID_NullSoundDriver)
...
// Implementation of csSoundDriverNullFactory

IMPLEMENT_UNKNOWN_NODELETE (csSoundDriverNullFactory)

BEGIN_INTERFACE_TABLE (csSoundDriverNullFactory)
...
STDMETHODIMP csSoundDriverNullFactory::CreateInstance (REFIID riid, ISystem* piSystem, void** ppv)
{
  ...
  csSoundDriverNull* pNew = new csSoundDriverNull (piSystem);
  if (!pNew)
  {
    *ppv = 0;
...
STDMETHODIMP csSoundDriverNullFactory::LockServer (COMBOOL bLock)
{
  if (bLock)
    gRefCount++;
  else
