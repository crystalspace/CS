...
void SoundDriverWaveOutRegister ()
{
  static csSoundDriverWaveOutFactory gFactory;
  gRegData.pClass = &gFactory;
  csRegisterServer (&gRegData);
}

void SoundDriverWaveOutUnregister ()
{
...
#define DLL_NAME "SoundDriverWaveOut.dll"
...
{
  static csSoundDriverWaveOutFactory gNullFactory;
  if (rclsid == CLSID_waveOutSoundDriver)
...
// Implementation of csSoundDriverWaveOutFactory

IMPLEMENT_UNKNOWN_NODELETE (csSoundDriverWaveOutFactory)

BEGIN_INTERFACE_TABLE (csSoundDriverWaveOutFactory)
...
STDMETHODIMP csSoundDriverWaveOutFactory::CreateInstance (REFIID riid, iSystem* piSystem, void** ppv)
{
  ...
  csSoundDriverWaveOut* pNew = new csSoundDriverWaveOut (piSystem);
  if (!pNew)
  {
    *ppv = 0;
...
STDMETHODIMP csSoundDriverWaveOutFactory::LockServer (COMBOOL bLock)
{
  if (bLock)
    gRefCount++;
  else
