...
class SndSysOggSoundStream : public iSndSysStream
{
 public:
  ...
  SndSysOggSoundStream(csRef<SndSysOggSoundData> data, OggDataStore *datastore, SndSysSoundFormat *renderformat, int mode3d);
   ...
   *  ISOUND2_STREAM_PAUSED - The stream is paused.
   *  ISOUND2_STREAM_UNPAUSED - The stream is not paused.  AdvancePosition is moving the stream position.
   */
  virtual int GetPauseState();

  /** Sets the loop state of the stream. Current acceptable values are ISOUND2_STREAM_DONTLOOP and ISOUND2_STREAM_LOOP
   *  May return FALSE if looping is not supported
   */
  virtual bool SetLoopState(int loopstate);

  /// Retrieves the loop state of the stream.  Current possible returns are ISOUND2_STREAM_DONTLOOP and ISOUND2_STREAM_LOOP
