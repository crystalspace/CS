...
class SndSysOggSoundStream : public iSndSysStream
{
 public:
   ...
   * May return ISOUND_STREAM_UNKNOWN_LENGTH if the stream is of unknown 
   * length.
   * For example, sound data being streamed from a remote host may not have a 
   * pre-determinable length.
   */
   ...
   * ISOUND_STREAM_UNKNOWN_LENGTH since an object implementing this interface 
   * should know its position relative to the begining of the data.  In the 
   * case where the begining may be ambiguous it should be considered to be at 
   * the point where the stream first started.  In other words, where there is 
   * doubt, the position should start at 0 and advance as the position advances.
   */
   ...
   * FALSE may be returned if a set position operation is not permitted or not 
   * possible.
   */
   ...
   * this call were not made.
   * For example, a stream coming from a remote host may not be able to be 
   * stopped or advance-buffered properly, in this case pausing the stream
   ...
   * ISNDSYS_STREAM_UNPAUSED - The stream is not paused.  AdvancePosition is 
   ...
   * "current time". A Sound Element will usually store the last advance time 
   * internally.  When this function is called it will compare the last time 
   * with the time presented and retrieve more data from the associated 
   * iSndSysData container as needed.  It will then update its internal last 
   * advance time value.
   *  
   * This function is not necessarily thread safe and must be called ONLY from 
   * the Sound System's main processing thread.
   */
   ...
   *	Sound System internally when a Source is created - through a call to 
   ...
   *	pointer to the first chunk of data on return or NULL (0) if no data is 
   *	available
   ...
   *
   */
  virtual void GetDataPointers (long *position_marker, 
    long max_requested_length, void **buffer1, long *buffer1_length, 
    void **buffer2, long *buffer2_length);
 ...
 protected:
  ...
  /**
   * If this value is >=0 then the next stream advancement will occur from the 
   * position requested. Used by ResetPosition()
   */
  long new_position;
