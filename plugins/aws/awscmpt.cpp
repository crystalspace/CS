...
bool awsMultiProctexCanvas::awscG2D::Initialize (iObjectRegistry *r)
{
  ...
  csReport (
    ...
    "crystalspace.graphics2d.awsMultiProctexCanvas::awscG2D",
    "awsMultiProctexCanvas::awscG2D mode %dx%d.",
    Width,
    Height);
...
void awsMultiProctexCanvas::awscG2D::SetClipRect (
  int xmin,
  int ymin,
  int xmax,
  int ymax)
  ...
  int color)
{
  ...
  for (i = 0; i < count; ++i)
  {
    ...
    if (rect.Intersects (crect))
    {
      awsSimpleCanvas *canvas = awsc->GetFlatCanvas (i);
      if (
        ((y1 - crect.ymin) < crect.Height ()) ||
        ((y2 - crect.ymin) < crect.Height ()))
      {
...
void awsMultiProctexCanvas::awscG2D::DrawBox (
  int x,
  int y,
  int w,
  int h,
...
void awsMultiProctexCanvas::awscG2D::Write (
  iFont *font,
  int x,
  int y,
  int fg,
  ...
  const char *text)
{
  ...
  if (!textRect.Intersects (clipRect)) return ;
...
void awsMultiProctexCanvas::awscG2D::WriteBaseline (
  iFont *font,
  int x,
  int y,
  int fg,
...
void awsMultiProctexCanvas::awscG2D::Print (csRect *area)
{
  ...
  for (i = 0; i < count; ++i)
  {
    ...
    {
      awsSimpleCanvas *canvas = awsc->GetFlatCanvas (i);
      awsc->realG3D ()->DrawPixmap (
          canvas->GetTextureWrapper ()->GetTextureHandle (),
          rect.xmin,
          rect.ymin,
          rect.Width () + 1,
          rect.Height () + 1,
...
{
  if ((width == height) && (width == -1)) return ;
...
{
  if ((G2D->GetWidth () != width) || (G2D->GetHeight () != height))
    SetDimensions (G2D->GetWidth (), G2D->GetHeight ());

  // if 2D graphics is not locked, lock it
  if (
    (DrawFlags & (CSDRAW_2DGRAPHICS | CSDRAW_3DGRAPHICS)) &&
    (!(DrawMode & (CSDRAW_2DGRAPHICS | CSDRAW_3DGRAPHICS))))
  {
    if (!G2D->BeginDraw ()) return false;
  }

  if (DrawFlags & CSDRAW_CLEARSCREEN) G2D->Clear (0);
...
void awsMultiProctexCanvas::awscG3D::DrawPolygon (G3DPolygonDP &

/*poly*/ )
{
}

void awsMultiProctexCanvas::awscG3D::DrawPolygonFX (G3DPolygonDPFX &

/*poly*/ )
{
}

bool awsMultiProctexCanvas::awscG3D::SetRenderState (
  G3D_RENDERSTATEOPTION op,
  long value)
{
  switch (op)
  {
    case G3DRENDERSTATE_ZBUFFERMODE:  z_buf_mode = csZBufMode (value); break;
...
long awsMultiProctexCanvas::awscG3D::GetRenderState (G3D_RENDERSTATEOPTION op)
{
  switch (op)
  {
    case G3DRENDERSTATE_ZBUFFERMODE:  return z_buf_mode;
    default:                          return 0;
  }
}

void awsMultiProctexCanvas::awscG3D::DrawLine (
  const csVector3 &,
  const csVector3 &,
  float
...
void awsMultiProctexCanvas::awscG3D::DrawPixmap (
  iTextureHandle *hTex,
  int sx,
  int sy,
  int sw,
  ...
  uint8 Alpha)
{
  ...
  for (i = 0; i < count; ++i)
  {
    ...
    if (!rect.IsEmpty ())
    {
      ...
      int ntx = tx + QInt (
          (((float)rect.xmin - (float)sx) / (float)sw) * (float)tw);
      int nty = ty + QInt (
          (((float)rect.ymin - (float)sy) / (float)sh) * (float)th);
    ...
    rect_list(0)
{
  ...
  while ((MaxCanvasHeight >> 1) >= DesiredCanvasHeight)
    MaxCanvasHeight >>= 1;
...
void awsMultiProctexCanvas::Animate (csTicks current_time)
{
  (void)current_time;
}
