...
awsClipper::awsClipper (iGraphics3D *_g3d, iGraphics2D *_g2d) :
  g3d(_g3d),
  g2d(_g2d)
{
}
...
void awsClipper::SetClipRect (csRect &r)
{
  clip.Set (r);
}

void awsClipper::DrawLine (float x1, float y1, float x2, float y2, int color)
{
  if (
    g2d->ClipLine (
        x1,
...
void awsClipper::DrawBox (int x, int y, int w, int h, int color)
{
...
void awsClipper::DrawPixel (int x, int y, int color)
{
  if (clip.Contains (x, y)) g2d->DrawPixel (x, y, color);
}

void awsClipper::Write (
  iFont *font,
  int x,
  int y,
  int fg,
