...
void awsSimpleCanvas::SetSize (int w, int h)
{
  mat_w = w;
  mat_h = h;
}
...
void awsSingleProctexCanvas::Animate (csTicks current_time)
{
  (void)current_time;
}

void awsSingleProctexCanvas::Show (
  csRect *area,
  iGraphics3D *g3d,
  uint8 Alpha)
{
  ...
  int w =
    (
      (!g3d) ||
      (G2D ()->GetWidth () < g3d->GetWidth ()) ? G2D ()->GetWidth () : g3d->GetWidth ()
    );
  int h =
    (
      (!g3d) ||
      (G2D ()->GetHeight () < g3d->GetHeight ()) ? G2D ()->GetHeight () : g3d->GetHeight ()
    );
