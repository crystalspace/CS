...
void csTextureHandleNull::remap_texture (csTextureManager *texman)
{
  ...
  switch (texman->pfmt.PixelBytes)
  {
    case 1:
      ...
      for (i = 0; i < palette_size; i++)
      {
        ((uint8 *)pal2glob)[i] = txm->cmap.find_rgb (
            palette[i].red,
            palette[i].green,
            palette[i].blue);
        pal2glob8[i] = txm->encode_rgb (
            palette[i].red,
            palette[i].green,
            palette[i].blue);
      }
...
void csTextureManagerNull::SetPixelFormat (csPixelFormat &PixelFormat)
{
  ...
  if ((pfmt.PixelBytes == 1) && !GlobalCMap) GlobalCMap = new uint16[256];
}
...
void csTextureManagerNull::read_config (iConfigFile *config)
{
  ...
  prefered_dist = config->GetInt (
      "Video.Null.TextureManager.RGBDist",
      PREFERED_DIST);
...
void csTextureManagerNull::create_inv_cmap ()
{
  ...
  inv_cmap[encode_rgb (
      cmap[0].red,
      cmap[0].green,
      cmap[0].blue)] = cmap.find_rgb (cmap[0].red, cmap[0].green, cmap[0].blue);
...
void csTextureManagerNull::compute_palette ()
{
  ...
  for (i = textures.Length () - 1; i >= 0; i--)
  {
    ...
    if (txt->GetAlphaMap ()) colormap++, colormapsize--;
  ...
  int colors = 0;
  for (i = 0; i < 256; i++)
    if (!locked[i] && cmap.alloc[i]) new_cmap[colors++] = cmap[i];
...
iTextureHandle *csTextureManagerNull::RegisterTexture (
  iImage *image,
  int flags)
{
...
{
  if (!truecolor && !palette_ok) compute_palette ();
