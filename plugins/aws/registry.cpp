...
void 
registry::addChild(const std::string &category, csRef<registry> _child) 
{ 
...
csRef<registry>
registry::findChild(const std::string &category, const std::string &name)
{
	...
	if (pos!=children.end())
	{
		for(child_list_type::iterator pos2 = pos->second.begin(); pos2!=pos->second.end(); ++pos2)
		{
			reg=*pos2;
			if (reg->Name() == name)
				return reg;
