    ...
    uint32 timestamp, uint32 frustumMask)
{
  ...
  {
    if (pvs[i]->child->timestamp != timestamp) 
    {
      pvs[i]->child->timestamp = timestamp;
    ...
    csString* set2, int set2count, int& resultCount)
{
  ...
  csString *result = new csString[set1count];
  ...
  csString *slimResult = new csString[resultCount];
  memcpy(slimResult, result, resultCount * sizeof(csString));
  delete[] result;
  return slimResult;
}

static void RemovePVS(csString*& set, int& setCount,
    csString* removeset, int removesetCount)
{
//  int count = 
  // TODO
}

static void CreateNodeData(csStaticKDTree *node)
{
  csString *pvs;
  int pvsCount;
  ...
  if (!node->IsLeafNode())
  {
    ...
    CreateNodeData (node->GetChild1 ());
    CreateNodeData (node->GetChild2 ());

    csPVSNodeData *wrapper1 = 
      (csPVSNodeData*) node->GetChild1 ()->GetNodeData ();
    csPVSNodeData *wrapper2 = 
      (csPVSNodeData*) node->GetChild2 ()->GetNodeData ();
    pvs = IntersectPVS(wrapper1->pvsnames, wrapper1->numTotal,
        wrapper2->pvsnames, wrapper2->numTotal, pvsCount);

    // TODO:  remove common items from both sets
  }
...
bool csPVSVis::Initialize (iObjectRegistry *object_reg)
{
  ...
  //desc->IncRef ();  // TODO:  used to be DecRef, was I right to change it?

  csRef<iGraphics2D> g2d = csQueryRegistry<iGraphics2D> (object_reg);
  if (g2d)
  {
    // TODO:  was it OK to change from g3d->GetX to g2d->GetX?
    ...
    // If there is no g2d we currently assume we are testing.
    scr_width = 640;
    scr_height = 480;
  }
    ...
    csStaticKDTreeObject* object)
{
  ...
  // nodes if the PVS name for the static object is found.
  node->GetNodeBBox() += object->GetBBox();
  if (node->IsLeafNode ()) 
  {
  ...
  else 
  {
    ...
    csPVSVisObjectWrapper *visobjWrapper = 
      ((csPVSVisObjectWrapper*) object->GetObject ());

    CS_ASSERT(nodeData);
    CS_ASSERT(visobjWrapper);

    const char *name = 
      visobjWrapper->visobj->GetMeshWrapper ()->QueryObject ()->GetName ();

    if (nodeData->PVSNamesContains(name))
    {
      // Object is contained in PVS set.  If all objects have been reg'd
      // in this node, then free up the pvsnames list.
      nodeData->pvs[nodeData->numRegistered++] = visobjWrapper;
      if (nodeData->numRegistered == nodeData->numTotal) 
      {
...
void csPVSVis::RegisterVisObject (iVisibilityObject* visobj)
{
  ...
  csPVSVisObjectWrapper* visobj_wrap = new csPVSVisObjectWrapper (this);
  ...
  csFlags flags = 
    visobj_wrap->visobj->GetMeshWrapper ()->GetMeshObject ()->GetFlags ();
  if (flags.CheckAll(CS_MESH_STATICPOS | CS_MESH_STATICSHAPE))
  {
    // Object is static, so we use our own static object inserter.
    //visobj_wrap->isStatic = true;
    visobj_wrap->child = new csStaticKDTreeObject(bbox, (void*)visobj_wrap);
    AddStaticObject (kdtree, visobj_wrap->child);
  }
  else
  {
    // If object is not static, then we will need to use frustum culling.
    iMovable* movable = visobj->GetMovable ();
...
//======== New Templated VisTest! ==========================================
/*
struct FrustTestCameraData;
static void CallVisibilityCallbacksForSubtree(csKDTree* node, FrustTestCameraData* data, uint32 cur_timestamp);
...
struct FrustTestCameraData
{
  ...
  bool NodeVisible(csKDTree* node, uint32 cur_timestamp, uint32& frustum_mask) 
  {
    int nodevis = viscull->TestNodeVisibility (node, this, frustum_mask);
    if (nodevis == NODE_INVISIBLE)
      return false;
    else if (nodevis == NODE_VISIBLE && frustum_mask == 0) 
    {
      ...
      // tree manually from this point on. To stop the traversal
      // we return false here.
      CallVisibilityCallbacksForSubtree (node, this, cur_timestamp);
      return false;
    }
    else
      return true;
  ...
  void TestFrustumVisibility(csKDTreeChild* object, uint32 cur_timestamp, uint32 frustum_mask) 
  {
    csPVSVisObjectWrapper* visobj_wrap = (csPVSVisObjectWrapper*)
      object->GetObject ();
    viscull->TestObjectVisibility (visobj_wrap, this, frustum_mask);
  }
};

template <class Data>
void FrustTest_Traverse (csKDTree* treenode, Data* data, uint32 cur_timestamp, uint32& frustum_mask)
{
  ...
  if (!treenode)
    return;
  if (!data->NodeVisible (treenode, cur_timestamp, frustum_mask))
    return;
  ...
  for (i = 0 ; i < num_objects ; i++)
  {
    if (objects[i]->timestamp != cur_timestamp)
    {
      ...
      data->TestFrustumVisbility(objects[i], cur_timestamp, frustum_mask);
    }
  }

  FrustTest_Traverse<Data> (treenode->GetChild1 (), data, cur_timestamp, frustum_mask);
  FrustTest_Traverse<Data> (treenode->GetChild2 (), data, cur_timestamp, frustum_mask);
}
*/
...
// Informs every object in this subtree that it is visible.
/*static void CallVisibilityCallbacksForSubtree(csStaticKDTree* node, FrustTestCameraData* data, uint32 cur_timestamp) {
    if (node->isLeafNode()) {
        csArray<csStaticKDTree*>::Iterator it = node->GetObjectIterator();
        while (it.HasNext()) {
            csStaticKDTreeObject* object = it.Next();
            if (object->timestamp != cur_timestamp)
            {
              object->timestamp = cur_timestamp;
              csPVSVisObjectWrapper* visobj_wrap = (csPVSVisObjectWrapper*) object->GetObject();
        ...
        CallVisibilityCallbacksForSubtree(node->GetChild1(), data, cur_timestamp);
        CallVisibilityCallbacksForSubtree(node->GetChild2(), data, cur_timestamp);
    }
} */
        ...
        FrustTestCameraData* data, uint32 cur_timestamp)
{
  ...
  FlagVisible(treenode, data->viscallback, cur_timestamp, 0);
...
// PVS visibility
/*static void PVSTestCamera_Traverse(csStaticKDTree* node, FrustTestCameraData* data, 
        uint32 cur_timestamp) {
} */

// Frustum visibility
/*void csPVSVis::FrustTestCamera_Traverse(csStaticKDTree* node, FrustTestCameraData* data, 
        uint32 cur_timestamp, uint32 frustum_mask) {
    int nodevis = TestNodeVisibility (treenode, data, frustum_mask);
    if (nodevis == NODE_INVISIBLE)
      return;
    else if (nodevis == NODE_VISIBLE && !frustum_mask) {
        CallVisibilityCallbacksForSubtree(node, data, cur_timestamp);
        return;
    }
    ...
    if (node->IsLeaf()) {
        ...
        FrustTest_Traverse(node->GetChild1(), data, cur_timestamp, frustum_mask);
        FrustTest_Traverse(node->GetChild2(), data, cur_timestamp, frustum_mask);
    }
} */
...
//  treenode->Distribute ();
  ...
  FlagVisible(treenode, data->viscallback, cur_timestamp, frustum_mask);
...
//  FrustTest_Traverse<FrustTestCameraData> (kdtree, &data, kdtree->NewTraversal (), frustum_mask);
...
/*  bool NodeVisible(csKDTree* node, uint32 cur_timestamp, uint32& frustum_mask) 
  {
    // In the first part of this test we are going to test if the node
    // itself is visible. If it is not then we don't need to continue.
    const csBox3& node_bbox = node->GetNodeBBox ();
    uint32 new_mask;
    if (!csIntersect3::BoxFrustum (node_bbox, frustum, frustum_mask, new_mask))
      return false;
    else {
      frustum_mask = new_mask;
      return true;
  ...
  void TestFrustumVisibility(csKDTreeChild* object, uint32 cur_timestamp, uint32& frustum_mask) 
  {
    ...
    uint32 new_mask2;
    if (csIntersect3::BoxFrustum (obj_bbox, frustum, frustum_mask, new_mask2))
    {
      if (viscallback)
        viscallback->ObjectVisible (visobj_wrap->visobj, visobj_wrap->mesh, new_mask2);
      else
        vistest_objects->Push (visobj_wrap->visobj);
    }
  } */
};
...
//  FrustTest_Traverse<FrustTestPlanes_Front2BackData> (kdtree, &data, kdtree->NewTraversal (), frustum_mask);
...
csPtr<iVisibilityObjectIterator> csPVSVis::VisTest (const csBox3& box)
{
  ...
  // TODO:  does this NEED to be front2back?
//  FrustTest_Traverse (kdtree, data, kdtree->NewTraversal (), 0);
...
//  {
    ...
    // need to con2tinue.
//    const csBox3& node_bbox = node->GetNodeBBox ();
//    if (!csIntersect3::BoxSphere (node_bbox, pos, sqradius))
//      return false;
//    else
//      return true;
//  }
...
//      object->GetObject ();
    ...
    // Test the bounding box of the object.
//    const csBox3& obj_bbox = visobj_wrap->child->GetBBox ();
//    if (csIntersect3::BoxSphere (obj_bbox, pos, sqradius))
//    {
//      if (viscallback)
//        viscallback->ObjectVisible (visobj_wrap->visobj, visobj_wrap->mesh, 0xff);
//      else
//        vistest_objects->Push (visobj_wrap->visobj);
//    }
//  }
...
csPtr<iVisibilityObjectIterator> csPVSVis::VisTest (const csSphere& sphere)
{
  ...
  data.viscallback = 0;
  // TODO:  does this NEED to be front2back?
//  FrustTest_Traverse<FrustTestSphere_Front2BackData> (kdtree, &data, kdtree->NewTraversal (), frustum_mask);
...
/*  bool NodeVisible(csKDTree* node, uint32 cur_timestamp, uint32& frustum_mask) 
  {
    ...
    // need to continue.
    // HACK:  box_isect takes advantage of the fact that NodeVisible is called, then TestFrustumVis a lot of times
    if (csIntersect3::BoxSegment (node_bbox, seg, box_isect) == -1)
      return false;
    else
     return true;
  }
