...
namespace aws
{
  namespace fvg
  {
    ...
    shape_attr * fvg_parser::FillAttribute (csRef<iDocumentNode> &pos, 
      ...
      while (pos->HasNext ())
      {
        ...
        else if (vo!=0)
        {
          ...
          else if(name=="rect")
          {
            ...
            if (roundness>0)	    
              vo->AddShape (new rect(RECT_ROUNDED, _attr, csVector2 (x,y), 
                                    csVector2 (x+width, y+height), roundness));
            else if (miter>0)
              vo->AddShape (new rect(RECT_MITERED, _attr, csVector2 (x,y), 
                                    csVector2 (x+width, y+height), miter));
            else
              vo->AddShape (new rect(RECT_NORMAL, _attr, csVector2(x,y), 
                                    csVector2 (x+width, y+height), 0));	    
          }
          else if(name=="circle")
          {
            float x,y,r;
